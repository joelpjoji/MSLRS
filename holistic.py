...
# LANDMARK_COUNT = HAND_LANDMARK_COUNT + POSE_LANDMARK_COUNT
...
def process_capture(cap, use_holistic):
    if use_holistic:
        ...
        solution = mp.solutions.hands.Hands(
            min_detection_confidence=0.8, min_tracking_confidence=0.6, max_num_hands=1)
    while(cap.isOpened()):
        ret, frame = cap.read()
        if ret == True:
            # BGR 2 RGB
...
def draw_landmarks(image, landmarks, use_holistic, tag=''):
    ...
    if use_holistic:
        mp_drawing.draw_landmarks(image, landmarks.left_hand_landmarks, mp.python.solutions.holistic.HAND_CONNECTIONS)
        if landmarks.left_hand_landmarks:
            cv2.putText(image, "L", (int(width*landmarks.left_hand_landmarks.landmark[0].x), int(height*landmarks.left_hand_landmarks.landmark[0].y)), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        ...
        if landmarks.multi_hand_landmarks:
            ...
            for hand_landmarks in landmarks.multi_hand_landmarks:
                # mp_drawing.draw_landmarks(image, hand_landmarks, mp.python.solutions.holistic.HAND_CONNECTIONS)
                mp_drawing.draw_landmarks(image, hand_landmarks, mp.python.solutions.hands.HAND_CONNECTIONS)
...
def normalize_features(features):
    ...
    # return np.concatenate((rh.flatten(), ra))
