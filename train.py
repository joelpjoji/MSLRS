...
def build_model(labels, frame_dim, dropout=0.3):
    ...
    model.add(layers.LSTM(128, name="lstm1",
...
def add_gesture_zero(dataset, num_labels):
    ...
    zeros = [(np.zeros((TRAIN_TIMESTEPS, dataset[0][0].shape[1])), 0)
             ] * len_per_label
...
def load_and_process_data(dirname):
    ...
    # Comment out this line to exclude "None" data when training
    data_iter = add_none_data(data_iter, dirname)
...
# Usage: [data_dir] [model_folder_name]
#eg: train.py output model
